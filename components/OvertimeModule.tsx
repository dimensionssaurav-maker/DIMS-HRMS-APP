...
import { distributeOTAcrossSlabs, totalOTPayFromSlabs, OTSlabResult } from '../utils/otSlabCalculator';
...
function slabColour(name: string) {
  return SLAB_COLOURS[name] ?? { bg: 'bg-indigo-100', text: 'text-indigo-800' };
}
...
const OvertimeModule: React.FC<Props> = ({
  ...
  const getShift = (emp: Employee): Shift | undefined =>
  ...
  const filteredData = useMemo(() => {
    ...
    const joined = recordsInPeriod.map(r => {
      ...
      if (shift?.otSlabs && shift.otSlabs.length > 0) {
        slabBreakdown   = distributeOTAcrossSlabs(r.overtimeHours, shift.endTime, shift.otSlabs, hourlyRate);
  ...
  const employeeStats = useMemo(() => {
    ...
    filteredData.forEach(d => {
      ...
      const e = map.get(d.employeeId)!;
      e.hours += d.overtimeHours; e.cost += d.totalCost;
    });
  ...
  const handleExport = (type: 'csv' | 'monthly') => {
    ...
    setTimeout(() => {
      ...
      if (type === 'csv') {
        const headers = ['Date', 'Emp ID', 'Name', 'Department', 'Actual OT Hrs', 'OT Pay', 'Fooding', 'Total',
                         ...allSlabNames.flatMap(s => [`${s} Hrs`, `${s} Pay`])];
        const rows = filteredData.map((d: any) => {
          const slabCols = allSlabNames.flatMap(sName => {
            const slab = d.slabBreakdown.find((s: OTSlabResult) => s.slabName === sName);
            return [slab?.hours ?? 0, slab?.amount ?? 0];
          });
          return [d.date, d.employeeId, `"${d.employeeName}"`, `"${d.department}"`,
                  d.overtimeHours, d.otAmount, d.foodingAmount, d.totalCost, ...slabCols];
        });
        const csv = [headers.join(','), ...rows.map(r => r.join(','))].join('\n');
        const blob = new Blob([csv], { type: 'text/csv' });
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob);
        a.download = `OT_Detail_${startDate}_${endDate}.csv`; a.click();
      } else {
        ...
        filteredData.forEach((d: any) => {
          ...
          e.days++; e.otHrs += d.overtimeHours; e.otPay += d.otAmount; e.fooding += d.foodingAmount; e.total += d.totalCost;
          (d.slabBreakdown as OTSlabResult[]).forEach(s => {
            if (!e.slabs[s.slabName]) e.slabs[s.slabName] = { hrs: 0, amt: 0 };
            e.slabs[s.slabName].hrs += s.hours; e.slabs[s.slabName].amt += s.amount;
          });
        });
        const headers = ['Emp ID', 'Name', 'Department', 'OT Days', 'Total OT Hrs', 'OT Pay', 'Fooding', 'Grand Total',
                         ...allSlabNames.flatMap(s => [`${s} Hrs`, `${s} Pay`])];
        ...
        const totalRow = ['', 'TOTAL', '', Array.from(empMap.values()).reduce((s: number, e: any) => s + e.days, 0),
                          Array.from(empMap.values()).reduce((s: number, e: any) => s + e.otHrs, 0).toFixed(2),
                          stats.totalOtCost, stats.totalFoodingCost, stats.totalCost,
                          ...allSlabNames.flatMap(sName => { const s = slabSummary.find(x => x.name === sName); return [s?.hours.toFixed(2) ?? 0, s?.amount ?? 0]; })];
        const csv = [headers.join(','), ...dataRows.map(r => r.join(',')), totalRow.join(',')].join('\n');
        const blob = new Blob([csv], { type: 'text/csv' });
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob);
        a.download = `OT_Monthly_Summary_${startDate}_${endDate}.csv`; a.click();
      }
      setIsExporting(null);
    }, 600);
  };
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
        <div className="flex items-center gap-2 bg-slate-50 p-2 rounded-xl border border-slate-200">
          ...
          <input type="date" value={startDate} onChange={e => onDateChange(e.target.value, endDate)}
            className="bg-white border border-slate-200 text-sm font-bold text-slate-700 py-1.5 px-3 rounded-lg outline-none focus:ring-2 focus:ring-indigo-500 cursor-pointer shadow-sm min-w-[140px]" />
        ...
        <div className="flex flex-wrap gap-3 w-full md:w-auto">
          ...
          <div className="flex items-center gap-2 bg-white p-1 rounded-xl border border-slate-200">
            ...
            <select value={departmentFilter} onChange={e => setDepartmentFilter(e.target.value)}
              className="bg-transparent text-sm font-semibold text-slate-700 py-1.5 pr-4 pl-2 outline-none cursor-pointer">
          ...
          <button onClick={() => handleExport('monthly')} disabled={!!isExporting}
            className="px-4 py-2 bg-indigo-600 text-white rounded-xl text-sm font-bold flex items-center gap-2 hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-200 whitespace-nowrap">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
          ...
          <div className="flex items-center gap-2">
            ...
            <p className="text-3xl font-black text-slate-800">{stats.totalHours.toFixed(1)}</p>
      ...
      {slabSummary.length > 0 && (
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            <h4 className="font-bold text-slate-700 text-sm uppercase tracking-wide">OT Slab Breakdown — Period Total</h4>
          </div>
            ...
            {slabSummary.map(s => {
              ...
              return (
                <div key={s.name} className={`p-4 rounded-2xl ${c.bg}`}>
                  <p className={`text-xs font-bold ${c.text} mb-2 uppercase tracking-wider flex items-center gap-1`}>
                    {slabIcon(s.name)}{s.name}
                  </p>
          ...
          {chartData.length > 0 ? (
            <ResponsiveContainer width="100%" height={250}>
              <BarChart data={chartData}>
                ...
                <XAxis dataKey="name" axisLine={false} tickLine={false} fontSize={12} tickFormatter={v => v.split(' ')[0]} />
                ...
                <div key={emp.id} className="flex items-center justify-between gap-4">
                  ...
                  <div className="flex-1 space-y-1">
                    <div className="flex justify-between text-[10px] font-medium text-slate-500">
                      <span>{emp.hours.toFixed(1)} hrs</span><span>₹{emp.cost.toLocaleString()}</span>
                    </div>
                    <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
                      <div className="bg-indigo-500 h-full rounded-full" style={{ width: `${(emp.hours / maxTotalHours) * 100}%` }} />
                    </div>
                  </div>
                </div>
              ))}
          ...
          <table className="w-full text-left">
            <thead className="bg-white sticky top-0 z-10">
              <tr className="text-xs font-bold text-slate-500 uppercase border-b border-slate-100">
                ...
                <th className="px-6 py-4">Slab Split</th>
                  ...
                  <React.Fragment key={idx}>
                    <tr className="hover:bg-amber-50/30 transition-colors cursor-pointer" onClick={() => setExpandedRow(isExpanded ? null : rowKey)}>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex-1 h-1.5 bg-slate-100 rounded-full w-20 overflow-hidden">
                            <div className={`h-full rounded-full ${item.overtimeHours > 4 ? 'bg-rose-500' : item.overtimeHours > 2 ? 'bg-amber-500' : 'bg-emerald-500'}`}
                      ...
                      <td className="px-6 py-4">
                        {item.hasSlabs ? (
                          <div className="flex flex-wrap gap-1">
                            {(item.slabBreakdown as OTSlabResult[]).map((s, si) => {
                              const c = slabColour(s.slabName);
                              return (
                                <span key={si} className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${c.bg} ${c.text}`}>
                                  {slabIcon(s.slabName)}{s.hours}h
                                </span>
                              );
                            })}
                          </div>
                      ...
                      <td className="px-6 py-4 text-right font-medium text-slate-600">
                        {item.isOtAllowed ? `₹${item.otAmount.toLocaleString()}` : <span className="text-slate-300">₹0</span>}
                      </td>
                      ...
                      <td className="px-6 py-4 text-right">
                        <span className={`font-black ${item.totalCost > 1000 ? 'text-indigo-600' : 'text-slate-800'}`}>
                          {item.isOtAllowed ? `₹${item.totalCost.toLocaleString()}` : <span className="text-slate-300">₹0</span>}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-center">
                        {item.isOtAllowed
                          ? <span className="px-2 py-1 bg-emerald-100 text-emerald-700 rounded text-[10px] font-bold uppercase">Approved</span>
                          : <span className="px-2 py-1 bg-red-100 text-red-700 rounded text-[10px] font-bold uppercase flex items-center justify-center gap-1"><AlertCircle size={10} /> Disallowed</span>}
                      </td>
                      <td className="px-3 py-4 text-slate-400">
                        {item.hasSlabs && (isExpanded ? <ChevronUp size={14} /> : <ChevronDown size={14} />)}
                      </td>
                    </tr>
                        ...
                        <td colSpan={9} className="px-6 py-3">
                          <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                            {(item.slabBreakdown as OTSlabResult[]).map((s, si) => {
                              const c = slabColour(s.slabName);
                              return (
                                <div key={si} className={`flex items-center justify-between p-3 rounded-xl ${c.bg}`}>
                                  <div>
                                    <p className={`text-[10px] font-black uppercase tracking-wider ${c.text} flex items-center gap-1`}>{slabIcon(s.slabName)}{s.slabName}</p>
                                    <p className="text-xs text-slate-600 mt-0.5">{s.hours} hrs × {s.multiplier}x</p>
                                  </div>
                                  <p className="text-sm font-black text-slate-800">₹{s.amount.toLocaleString()}</p>
                                </div>
                              );
                            })}
                            <div className="flex items-center justify-between p-3 rounded-xl bg-slate-800 md:col-span-4 mt-1">
                              ...
                              <p className="text-sm font-black text-white">
                                {(item.slabBreakdown as OTSlabResult[]).reduce((s: number, b: OTSlabResult) => s + b.hours, 0).toFixed(2)} hrs → ₹{item.totalCost.toLocaleString()}
                              </p>
                            </div>
                          </div>
                        </td>
