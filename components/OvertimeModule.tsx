...
interface Props {
  ...
  onDateChange: (start: string, end: string) => void; payrollConfig: PayrollConfig;
}

const OvertimeModule: React.FC<Props> = ({ employees, attendanceRecords, departments, shifts, startDate, endDate, onDateChange, payrollConfig }) => {
  ...
  const filteredData = useMemo(() => {
    ...
    const joined = recordsInPeriod.map(r => {
      ...
      return { ...r, employeeName: emp.name, department: emp.department, designation: emp.designation, isOtAllowed: emp.isOtAllowed, multiplier, hourlyRate, payableHours, isTieredApplied, slabBreakdown, hasSlabs: slabBreakdown.length > 0, otAmount, foodingAmount, totalCost: otAmount + foodingAmount };
    }).filter(Boolean) as any[];
    return joined.filter(item => {
      const matchDept = departmentFilter === 'All Departments' || item.department === departmentFilter;
      const matchSearch = (item.employeeName ?? '').toLowerCase().includes(employeeSearch.toLowerCase()) || (item.employeeId ?? '').toLowerCase().includes(employeeSearch.toLowerCase());
  ...
  const slabSummary = useMemo(() => {
    ...
    filteredData.forEach(d => { (d.slabBreakdown as OTSlabResult[]).forEach(s => { const cur = map.get(s.slabName) ?? { hours: 0, amount: 0, multiplier: s.multiplier }; map.set(s.slabName, { hours: cur.hours + s.hours, amount: cur.amount + s.amount, multiplier: s.multiplier }); }); });
  ...
  const handleExport = (type: 'csv' | 'monthly') => {
    ...
    setTimeout(() => {
      ...
      if (type === 'csv') {
        ...
        const dataRows = Array.from(empMap.values()).sort((a,b)=>b.total-a.total).map(e => { const sc=allSlabNames.flatMap(s=>[e.slabs[s]?.hrs?.toFixed(2)??0,e.slabs[s]?.amt??0]); return [e.id,'"'+e.name+'"','"'+e.dept+'"',e.days,e.otHrs.toFixed(2),e.otPay,e.fooding,e.total,...sc]; });
        const tr=['','TOTAL','',Array.from(empMap.values()).reduce((s:number,e:any)=>s+e.days,0),Array.from(empMap.values()).reduce((s:number,e:any)=>s+e.otHrs,0).toFixed(2),stats.totalOtCost,stats.totalFoodingCost,stats.totalCost,...allSlabNames.flatMap(sn=>{const s=slabSummary.find(x=>x.name===sn);return[s?.hours.toFixed(2)??0,s?.amount??0];})];
        const csv=[headers.join(','),...dataRows.map(r=>r.join(',')),tr.join(',')].join('\n');
        const a=document.createElement('a'); a.href=URL.createObjectURL(new Blob([csv],{type:'text/csv'})); a.download='OT_Monthly_'+startDate+'_'+endDate+'.csv'; a.click();
      }
      setIsExporting(null);
    }, 600);
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-amber-50 p-6 rounded-2xl border border-amber-100 col-span-2">
          <div className="flex justify-between items-start">
            <div><p className="text-xs font-bold text-amber-600 uppercase tracking-widest mb-1">Total OT + Fooding Cost</p><div className="flex items-center gap-2"><IndianRupee size={28} className="text-amber-700"/><p className="text-4xl font-black text-slate-800">{stats.totalCost.toLocaleString()}</p></div></div>
      ...
      {slabSummary.length > 0 && (
        <div>
          <div className="flex items-center gap-2 mb-3"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-indigo-600"><polygon points="12 2 2 7 12 12 22 7 12 2"/><polyline points="2 17 12 22 22 17"/><polyline points="2 12 12 17 22 12"/></svg><h4 className="font-bold text-slate-700 text-sm uppercase tracking-wide">OT Slab Breakdown — Period Total</h4></div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {slabSummary.map(s => { const c = slabColour(s.name); return (<div key={s.name} className={"p-4 rounded-2xl "+c.bg}><p className={"text-xs font-bold "+c.text+" mb-2 uppercase tracking-wider"}>{s.name}</p><div className="flex justify-between items-end"><div><p className="text-xs text-slate-500">Hours</p><p className="text-xl font-black text-slate-800">{s.hours.toFixed(1)}</p></div><div className="text-right"><p className="text-xs text-slate-500">{s.multiplier}x</p><p className="text-xl font-black text-slate-800">₹{s.amount.toLocaleString()}</p></div></div></div>); })}
          </div>
        </div>
      )}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm min-h-[300px]">
          ...
          {chartData.length > 0 ? (<ResponsiveContainer width="100%" height={250}><BarChart data={chartData}><CartesianGrid strokeDasharray="3 3" vertical={false}/><XAxis dataKey="name" axisLine={false} tickLine={false} fontSize={12} tickFormatter={v=>v.split(' ')[0]}/><YAxis axisLine={false} tickLine={false} fontSize={12}/><Tooltip cursor={{fill:'#fef3c7'}} contentStyle={{borderRadius:'8px',border:'none'}}/><Bar dataKey="value" fill="#d97706" radius={[4,4,0,0]} barSize={40}/></BarChart></ResponsiveContainer>) : <div className="h-full flex items-center justify-center text-slate-400 text-sm">No data for range</div>}
        </div>
        <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm min-h-[300px] flex flex-col">
          ...
          {employeeStats.length > 0 ? (<div className="space-y-4 overflow-y-auto flex-1 pr-2">{employeeStats.slice(0,6).map((emp,i) => (<div key={emp.id} className="flex items-center justify-between gap-4"><div className="flex items-center gap-3 w-1/3"><div className={"w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold "+(i<3?'bg-amber-100 text-amber-700':'bg-slate-100 text-slate-500')}>{i+1}</div><div className="truncate"><p className="text-xs font-bold text-slate-700 truncate">{emp.name}</p><p className="text-[9px] text-slate-400 uppercase">{emp.department}</p></div></div><div className="flex-1 space-y-1"><div className="flex justify-between text-[10px] font-medium text-slate-500"><span>{emp.hours.toFixed(1)} hrs</span><span>₹{emp.cost.toLocaleString()}</span></div><div className="h-1.5 bg-slate-100 rounded-full overflow-hidden"><div className="bg-indigo-500 h-full rounded-full" style={{width:(emp.hours/maxTotalHours*100)+'%'}}/></div></div></div>))}</div>) : <div className="h-full flex items-center justify-center text-slate-400 text-sm">No data found</div>}
        </div>
      </div>
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-100 bg-slate-50 flex justify-between items-center"><h3 className="font-bold text-slate-700">Detailed OT Register</h3><span className="text-xs font-bold text-slate-400 bg-white px-2 py-1 rounded border border-slate-200">{filteredData.length} records</span></div>
        <div className="overflow-x-auto max-h-[600px]">
          <table className="w-full text-left">
            <thead className="bg-white sticky top-0 z-10"><tr className="text-xs font-bold text-slate-500 uppercase border-b border-slate-100"><th className="px-6 py-4">Date</th><th className="px-6 py-4">Employee</th><th className="px-6 py-4">OT Hours</th><th className="px-6 py-4">Slab Split</th><th className="px-6 py-4 text-right">OT Pay</th><th className="px-6 py-4 text-right">Fooding</th><th className="px-6 py-4 text-right">Total</th><th className="px-6 py-4 text-center">Status</th><th className="px-3 py-4"></th></tr></thead>
            <tbody className="divide-y divide-slate-50 text-sm">
              {filteredData.length === 0 ? (<tr><td colSpan={9} className="px-6 py-12 text-center text-slate-400 italic">No overtime records found for this period.</td></tr>)
              : filteredData.map((item: any, idx: number) => {
                const rowKey = item.employeeId+'-'+item.date; const isExpanded = expandedRow === rowKey;
                return (<React.Fragment key={idx}>
                  <tr className="hover:bg-amber-50/30 transition-colors cursor-pointer" onClick={()=>setExpandedRow(isExpanded?null:rowKey)}>
                    ...
                    <td className="px-6 py-4"><div className="flex items-center gap-3"><span className="font-bold text-slate-700 w-8 text-right">{item.overtimeHours}</span><div className="flex-1 h-1.5 bg-slate-100 rounded-full w-20 overflow-hidden"><div className={"h-full rounded-full "+(item.overtimeHours>4?'bg-rose-500':item.overtimeHours>2?'bg-amber-500':'bg-emerald-500')} style={{width:Math.min(item.overtimeHours/maxDailyHours*100,100)+'%'}}/></div>{item.overtimeHours>4&&<AlertTriangle size={14} className="text-rose-500"/>}</div></td>
                    <td className="px-6 py-4">{item.hasSlabs?(<div className="flex flex-wrap gap-1">{(item.slabBreakdown as OTSlabResult[]).map((s,si)=>{const c=slabColour(s.slabName);return(<span key={si} className={"text-[10px] font-bold px-2 py-0.5 rounded-full "+c.bg+" "+c.text}>{s.hours}h</span>);})}</div>):<span className="text-xs text-slate-400">Flat rate</span>}</td>
                    ...
                    <td className="px-6 py-4 text-right"><span className={item.totalCost>1000?'font-black text-indigo-600':'font-black text-slate-800'}>{item.isOtAllowed?'₹'+item.totalCost.toLocaleString():<span className="text-slate-300">₹0</span>}</span></td>
                  ...
                  {isExpanded&&item.hasSlabs&&(<tr className="bg-indigo-50/40"><td colSpan={9} className="px-6 py-3"><div className="grid grid-cols-1 md:grid-cols-4 gap-3">{(item.slabBreakdown as OTSlabResult[]).map((s,si)=>{const c=slabColour(s.slabName);return(<div key={si} className={"flex items-center justify-between p-3 rounded-xl "+c.bg}><div><p className={"text-[10px] font-black uppercase tracking-wider "+c.text}>{s.slabName}</p><p className="text-xs text-slate-600 mt-0.5">{s.hours} hrs × {s.multiplier}x</p></div><p className="text-sm font-black text-slate-800">₹{s.amount.toLocaleString()}</p></div>);})}<div className="flex items-center justify-between p-3 rounded-xl bg-slate-800 md:col-span-4 mt-1"><p className="text-xs font-bold text-slate-300 uppercase tracking-wide">Day Total</p><p className="text-sm font-black text-white">{(item.slabBreakdown as OTSlabResult[]).reduce((s:number,b:OTSlabResult)=>s+b.hours,0).toFixed(2)} hrs → ₹{item.totalCost.toLocaleString()}</p></div></div></td></tr>)}
                </React.Fragment>);
              })}
            </tbody>
          </table>
