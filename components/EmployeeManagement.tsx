...
const EmployeeManagement: React.FC<Props> = ({ employees, departments, shifts = [], payrollConfig, onAdd, onBulkAdd, onDelete, onUpdate }) => {
  ...
  const handleSaveEdit = () => {
    if (editingEmployee) {
      const updated = {
        ...editingEmployee,
        employeeCode: editingEmployee.employeeCode || editingEmployee.id,
        monthlyBase: editingEmployee.salaryType === 'Monthly' ? editingEmployee.monthlySalary : editingEmployee.dailyWage * 30
      };
      onUpdate([updated]);
    ...
    <div className="animate-in slide-in-from-bottom-4 duration-500">
      ...
      {editingEmployee && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[150] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl animate-in zoom-in-95 flex flex-col max-h-[92vh]">
            {/* Header */}
            <div className="flex justify-between items-center px-6 py-4 border-b border-slate-100 shrink-0">
              <h3 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                <Edit2 size={18} className="text-indigo-600" /> Edit Employee
                <span className="text-xs font-mono text-slate-400 bg-slate-100 px-2 py-0.5 rounded-lg ml-1">{editingEmployee.employeeCode || editingEmployee.id}</span>
              </h3>
              <button onClick={() => setEditingEmployee(null)} className="p-2 hover:bg-slate-100 rounded-full text-slate-400"><X size={20} /></button>
            </div>

            {/* Scrollable Body — ALL fields in one view */}
            <div className="overflow-y-auto p-6 space-y-6">
                ...
                <div className="relative group cursor-pointer shrink-0" onClick={() => fileInputRef.current?.click()}>
                  <div className="w-16 h-16 rounded-full overflow-hidden bg-slate-100 border-2 border-slate-200">
                    {editingEmployee.avatar
                      ? <img src={editingEmployee.avatar} alt="Profile" className="w-full h-full object-cover" />
              ...
              {/* Employee Code (display only) */}
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-bold text-slate-500 uppercase">Employee ID / Code</label>
                  <input type="text"
                    value={editingEmployee.employeeCode || editingEmployee.id}
                ...
                <div>
                  ...
                  <input type="date" value={editingEmployee.joiningDate}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={editingEmployee.department}
                    onChange={e => setEditingEmployee({...editingEmployee, department: e.target.value})}
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold outline-none mt-1">
                    {departments.filter(d => d !== 'All Departments').map(d => <option key={d} value={d}>{d}</option>)}
                    <option value={editingEmployee.department}>{editingEmployee.department}</option>
                  </select>
                </div>
                <div>
              ...
              <div className="bg-indigo-50 rounded-xl p-4 border border-indigo-100 space-y-4">
                <p className="text-xs font-bold text-indigo-700 uppercase tracking-widest flex items-center gap-1"><IndianRupee size={12} /> Salary & Wages</p>
                <div className="flex bg-white p-1 rounded-lg border border-indigo-200 w-fit">
                  <button onClick={() => setEditingEmployee({...editingEmployee, salaryType: 'Daily'})}
                    className={`px-5 py-1.5 rounded-md text-sm font-bold transition-all ${editingEmployee.salaryType === 'Daily' ? 'bg-indigo-600 text-white shadow' : 'text-slate-500'}`}>Daily</button>
                  <button onClick={() => setEditingEmployee({...editingEmployee, salaryType: 'Monthly'})}
                    className={`px-5 py-1.5 rounded-md text-sm font-bold transition-all ${editingEmployee.salaryType === 'Monthly' ? 'bg-indigo-600 text-white shadow' : 'text-slate-500'}`}>Monthly</button>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                  ...
                  <div>
                    ...
                    <div className="flex gap-2 mt-1">
                      <button onClick={() => setEditingEmployee({...editingEmployee, isOtAllowed: true})}
                        className={`flex-1 py-2.5 rounded-xl text-sm font-bold border transition-all ${editingEmployee.isOtAllowed ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white border-slate-200 text-slate-500'}`}>Yes</button>
                      <button onClick={() => setEditingEmployee({...editingEmployee, isOtAllowed: false})}
                        className={`flex-1 py-2.5 rounded-xl text-sm font-bold border transition-all ${!editingEmployee.isOtAllowed ? 'bg-red-500 text-white border-red-500' : 'bg-white border-slate-200 text-slate-500'}`}>No</button>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <select value={editingEmployee.shiftId || ''} onChange={e => setEditingEmployee({...editingEmployee, shiftId: e.target.value})}
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold outline-none mt-1">
              ...
              {/* Source & Service Charge — dropdowns from payrollConfig */}
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-bold text-slate-500 uppercase flex items-center justify-between">
                    Hiring Source
                    <span className="text-[9px] text-indigo-400 normal-case font-medium">Set in Settings → Payroll</span>
                  </label>
                  <select value={editingEmployee.source || ''}
                    onChange={e => setEditingEmployee({...editingEmployee, source: e.target.value})}
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold outline-none mt-1 focus:ring-2 focus:ring-indigo-500">
                    <option value="">— None —</option>
                    {(payrollConfig?.recruitmentConfig?.sources || []).map(src => (
                      <option key={src} value={src}>{src}</option>
                    ))}
                    {/* Keep current value selectable even if removed from config */}
                    {editingEmployee.source && !(payrollConfig?.recruitmentConfig?.sources || []).includes(editingEmployee.source) && (
                      <option value={editingEmployee.source}>{editingEmployee.source}</option>
                    )}
                  </select>
                </div>
                <div>
                  ...
                  <select value={editingEmployee.serviceChargeRate ?? ''}
                    onChange={e => setEditingEmployee({...editingEmployee, serviceChargeRate: e.target.value === '' ? undefined : Number(e.target.value)})}
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold outline-none mt-1 focus:ring-2 focus:ring-indigo-500">
                    <option value="">— None —</option>
                    {(payrollConfig?.recruitmentConfig?.serviceChargeRates || []).map(rate => (
                      <option key={rate} value={rate}>{(rate * 100).toFixed(2)}% ({rate})</option>
                    ))}
                    {/* Keep current value selectable if not in config list */}
                    {editingEmployee.serviceChargeRate != null &&
                     !(payrollConfig?.recruitmentConfig?.serviceChargeRates || []).includes(editingEmployee.serviceChargeRate) && (
                      <option value={editingEmployee.serviceChargeRate}>
                        {(editingEmployee.serviceChargeRate * 100).toFixed(2)}% (custom)
                      </option>
                    )}
                  </select>
                </div>
            ...
            <div className="px-6 py-4 border-t border-slate-100 flex gap-3 bg-slate-50 shrink-0">
              ...
              <button onClick={handleSaveEdit} className="flex-1 py-3 bg-indigo-600 text-white font-bold rounded-xl shadow-lg hover:bg-indigo-700 text-sm flex items-center justify-center gap-2">
