          ...
          <table className="w-full text-xs border-collapse">
            <thead>
              <tr className="bg-slate-800 text-white">
                <th className="px-3 py-3 text-left font-bold text-[11px] uppercase tracking-wider min-w-[80px]">EMP ID</th>
                <th className="px-3 py-3 text-left font-bold text-[11px] uppercase tracking-wider min-w-[160px]">Name</th>
                <th className="px-3 py-3 text-left font-bold text-[11px] uppercase tracking-wider min-w-[300px]">Status</th>
                <th className="px-3 py-3 text-left font-bold text-[11px] uppercase tracking-wider min-w-[110px]">In</th>
                <th className="px-3 py-3 text-left font-bold text-[11px] uppercase tracking-wider min-w-[110px]">Out</th>
                <th className="px-3 py-3 text-center font-bold text-[11px] uppercase tracking-wider min-w-[80px]">Hours</th>
                <th className="px-3 py-3 text-center font-bold text-[11px] uppercase tracking-wider min-w-[80px]">OT</th>
                <th className="px-3 py-3 text-left font-bold text-[11px] uppercase tracking-wider min-w-[110px]">Flags</th>
              </tr>
            </thead>
            <tbody>
              {filteredEmployees.length === 0 ? (
                <tr><td colSpan={8} className="py-12 text-center text-slate-400 font-medium">No employees match your search.</td></tr>
                ...
                return (
                  <tr key={emp.id} className={`border-b border-slate-100 hover:bg-indigo-50/30 transition-colors ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'}`}>
                    {/* EMP ID */}
                    <td className="px-3 py-2.5">
                      <span className="font-mono text-[11px] text-slate-500 font-bold">{emp.employeeCode || emp.id}</span>
                    </td>
                    {/* NAME */}
                    <td className="px-3 py-2.5">
                      ...
                      {isFullHoliday && (
                        <span className="text-[9px] font-bold text-purple-600 bg-purple-50 px-1.5 py-0.5 rounded mt-0.5 inline-block">
                          {isSundayOff ? 'Weekly Off' : activeHoliday?.name}
                        </span>
                      )}
                      {isSunday && isSundaySchedule && sundayConfig?.enabled && (
                        <span className="text-[9px] font-bold text-orange-600 bg-orange-50 px-1.5 py-0.5 rounded mt-0.5 inline-flex items-center gap-0.5">
                        ...
                        {statuses.map((s) => {
                          ...
                          return (
                            <button
                              ...
                              title={s.label}
                              className={`flex items-center gap-1 px-2 py-1 rounded-lg border text-[10px] font-bold transition-all hover:scale-105 ${isActive ? s.activeColor : s.inactiveColor + ' opacity-50 hover:opacity-100'}`}
                            >
                              <Icon size={11} />
                              <span>{s.label}</span>
                            </button>
                    ...
                    <td className="px-3 py-2.5">
                      <input type="time" value={record?.checkIn || ''} onChange={(e) => handleTimeChange(emp.id, 'checkIn', e.target.value)} disabled={isFullHoliday}
                        className={`border rounded-lg px-2 py-1 text-xs font-bold text-slate-700 outline-none focus:border-indigo-400 w-28 ${record?.lateMinutes && record.lateMinutes > 0 ? 'bg-rose-50 border-rose-200' : 'bg-white border-slate-200'} ${isFullHoliday ? 'opacity-40 cursor-not-allowed' : ''}`}
                      />
                    </td>
                    {/* OUT */}
                    <td className="px-3 py-2.5">
                      <input type="time" value={record?.checkOut || ''} onChange={(e) => handleTimeChange(emp.id, 'checkOut', e.target.value)} disabled={isFullHoliday}
                        className={`border rounded-lg px-2 py-1 text-xs font-bold text-slate-700 outline-none focus:border-indigo-400 w-28 ${record?.earlyMinutes && record.earlyMinutes > 0 ? 'bg-orange-50 border-orange-200' : 'bg-white border-slate-200'} ${isFullHoliday ? 'opacity-40 cursor-not-allowed' : ''}`}
                      />
                    </td>
                    {/* HOURS */}
                    <td className="px-3 py-2.5 text-center">
                      {duration ? (
                        <span className={`text-[11px] font-bold px-2 py-1 rounded-lg ${duration.totalHours > 9 ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'}`}>
                      ...
                      {emp.isOtAllowed ? (
                        <div className="flex items-center justify-center gap-1">
                          <input type="number" min="0" max="12" step="0.5"
                            ...
                            className="w-14 border border-slate-200 bg-slate-50 rounded-lg p-1 text-center text-xs font-bold text-indigo-600 outline-none focus:ring-1 focus:ring-indigo-500"
                          />
                          <span className="text-[10px] text-slate-400">h</span>
                        </div>
                      ) : (
                        <span className="text-[10px] text-slate-300 flex items-center justify-center gap-0.5"><Lock size={9}/> N/A</span>
                      )}
                    </td>
                    {/* FLAGS */}
                    <td className="px-3 py-2.5">
                      <div className="flex flex-col gap-1">
                        {record?.lateMinutes && record.lateMinutes > 0 ? (
                          <span className="bg-rose-100 text-rose-600 px-2 py-0.5 rounded text-[10px] font-bold">Late {record.lateMinutes}m</span>
                        ) : null}
                        {record?.earlyMinutes && record.earlyMinutes > 0 ? (
                          <span className="bg-orange-100 text-orange-600 px-2 py-0.5 rounded text-[10px] font-bold">Early {record.earlyMinutes}m</span>
                        ) : null}
                        {!record?.lateMinutes && !record?.earlyMinutes && record?.status === ('PRESENT' as AttendanceStatus) && (
                          <span className="text-emerald-500 text-[10px] font-bold">✓ On Time</span>
                        )}
                      </div>
                    </td>
                  </tr>
