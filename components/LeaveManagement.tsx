...
const LeaveManagement: React.FC<Props> = ({ employees, leaveRequests, holidays, onAddRequest, onUpdateRequest, onUpdateHolidays }) => {
  ...
  const entitlements = {
    ...
    ['Casual Leave']: 12,
    ['Earned Leave']: 15,
    ['Loss of Pay']: 0 // Infinite really
  };

  const getBalances = (empId: string) => {
    const empRequests = leaveRequests.filter(r => 
      r.employeeId === empId && r.status === 'Approved'
    );

    const consumed = {
      ['Sick Leave']: empRequests.filter(r => r.type === 'Sick Leave').reduce((acc, curr) => acc + curr.days, 0),
      ['Casual Leave']: empRequests.filter(r => r.type === 'Casual Leave').reduce((acc, curr) => acc + curr.days, 0),
      ['Earned Leave']: empRequests.filter(r => r.type === 'Earned Leave').reduce((acc, curr) => acc + curr.days, 0),
    };

    return {
      sick: Math.max(0, entitlements['Sick Leave'] - consumed['Sick Leave']),
      casual: Math.max(0, entitlements['Casual Leave'] - consumed['Casual Leave']),
      earned: Math.max(0, entitlements['Earned Leave'] - consumed['Earned Leave']),
    };
  };
