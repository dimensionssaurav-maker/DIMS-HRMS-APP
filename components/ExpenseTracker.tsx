...
const ExpenseTracker: React.FC<Props> = ({ claims, employees, departments, onAddClaim, onUpdateClaim }) => {
  ...
  const stats = useMemo(() => {
    ...
    const pending = claims.filter(c => c.status === ClaimStatus.PENDING);
    const approved = claims.filter(c => c.status === ClaimStatus.APPROVED);
    const reimbursed = claims.filter(c => c.status === ClaimStatus.REIMBURSED);
    const rejected = claims.filter(c => c.status === ClaimStatus.REJECTED);
  ...
  const getStatusColor = (status: ClaimStatus) => {
    switch (status) {
      ...
      case ClaimStatus.PENDING: return 'bg-amber-100 text-amber-700';
      case ClaimStatus.REJECTED: return 'bg-red-100 text-red-700';
      case ClaimStatus.DRAFT: return 'bg-slate-100 text-slate-600';
      case ClaimStatus.REIMBURSED: return 'bg-blue-100 text-blue-700';
                             ...
                             {openActionId === claim.id && (
                               <>
                                 ...
                                 <div className="absolute right-12 top-2 w-48 bg-white rounded-xl shadow-xl border border-slate-100 z-20 overflow-hidden animate-in fade-in zoom-in-95 duration-200 origin-top-right">
                                    {(claim.status === ClaimStatus.PENDING || claim.status === ClaimStatus.DRAFT) && (
                                       <>
                                         <button 
                                           onClick={() => handleStatusChange(claim, ClaimStatus.APPROVED)}
                                         ...
                                         <button 
                                           onClick={() => handleStatusChange(claim, ClaimStatus.REJECTED)}
                                    ...
                                    {claim.status === ClaimStatus.APPROVED && (
                                       <button 
                                         onClick={() => handleStatusChange(claim, ClaimStatus.REIMBURSED)}
