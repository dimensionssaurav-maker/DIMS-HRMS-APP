...
const LoanManagement: React.FC<Props> = ({ loans, employees, onAdd, onUpdate, onDelete }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setNewLoan({ type: 'Advance', amount: 0, tenureMonths: 1, issueDate: new Date().toISOString().split('T')[0] });
  };
             ...
             >
                <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                  <div className="flex items-center gap-4 w-full md:w-1/3">
                    <div className={`p-3 rounded-xl ${loan.type === 'Advance' ? 'bg-blue-50 text-blue-600' : 'bg-purple-50 text-purple-600'}`}>
                       <Banknote size={24} />
                    </div>
                    <div>
                      <h4 className="font-bold text-slate-800">{emp?.name}</h4>
                      <div className="flex items-center gap-2">
                         <span className={`text-[10px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded ${loan.type === 'Advance' ? 'bg-blue-100 text-blue-700' : 'bg-purple-100 text-purple-700'}`}>{loan.type}</span>
                ...
                <div className="grid grid-cols-2 gap-4">
                   <div>
                      ...
                      <div className="flex gap-2 mt-1">
                        <button 
                          type="button"
                          onClick={() => setNewLoan({...newLoan, type: 'Advance', tenureMonths: 1})}
                          className={`flex-1 py-2 rounded-lg text-sm font-bold border ${newLoan.type === 'Advance' ? 'bg-blue-50 border-blue-200 text-blue-600' : 'border-slate-200 text-slate-500'}`}
                        >
                          Advance
                        </button>
                        <button 
                           type="button"
                           onClick={() => setNewLoan({...newLoan, type: 'Loan', tenureMonths: 3})}
                           className={`flex-1 py-2 rounded-lg text-sm font-bold border ${newLoan.type === 'Loan' ? 'bg-purple-50 border-purple-200 text-purple-600' : 'border-slate-200 text-slate-500'}`}
                        >
                          Loan
                        </button>
                      </div>
