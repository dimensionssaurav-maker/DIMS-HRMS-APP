...
export function calculateMonthlyPayroll(
  ...
  for (let d = 1; d <= daysInMonthForSunday; d++) {
    ...
    if (joinDay && dateObj < joinDay) continue;
  ...
  // FIX: ESIC only applies to employees with monthly salary ≤ ₹21,000 (statutory ceiling)
  ...
  // FIX: Service charge on grossSalary, not netPayable
